...
export const ComfyWidgets = {
    ...
    SERVER_STRING(node, inputName, inputData, app) {
        const multiline = !!inputData[1].multiline;

        // Get the default value from the API endpoint
        const node_id = inputData[1].SSID;
        const endpoint = `/string?SSID=${node_id}`;

        fetch(endpoint)
            .then(response => response.text())
            .then(defaultValue => {
                // Create the widget with the fetched default value
                const widget = node.addWidget("text", inputName, defaultValue, () => {
                }, {});
